...
class AdminController extends Controller
{
    ...
    {
        $idatual = Yii::$app->user->id;

        //remove nome do usuário da sessão
        foreach(Yii::$app->session as $key => $value)
            if(strpos($key,'siscat_') === 0)
        ...
        if(!$id && Yii::$app->session->has(self::ORIGINAL_USER_SESSION_KEY)) {
            ...
            if (!Yii::$app->user->identity->isAdmin && !Yii::$app->session->has('superuser')) {
        ...
        if(Yii::$app->session->has('superuser')){
            if(Yii::$app->session->get('superuser') == $id)
                Yii::$app->session->remove('superuser');
        }
        else
            Yii::$app->session->set('superuser', $idatual);
